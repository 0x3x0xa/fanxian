...
class MemberController extends CommonController {
    ...
    public function index() {
        ...
        $shouyi=$member_table->sum('profit');
        $touzi=$total_table->sum('selftotalmoney');
        ...
        for ($i = 0; $i < $allcount; $i++) {
            ...
            $totalInfo=$total_table->where(array('uid'=>$list[$i]['id']))->find();
            $list[$i]['touzimoney']=  empty($totalInfo['selftotalmoney'])?0:$totalInfo['selftotalmoney'];
            if ($re_info) {
...
//启用用户
        ...
        if (IS_AJAX) {
            ...
            switch ($type) {
                case 1:
                    ...
                    break;
                case 2:
                    $money = $userInfo['profit'] + $income;
                    $rel1 = $member_table->save(array('id' => $userInfo['id'], 'profit' => $money));
                    break;
                default :
                    showMsg(2, '币种不存在');
            }
            $rel2 = $bonus_table->add(array('uid' => $userInfo['id'],'pursetype'=>$type, 'type' => 5, 'sum' => $income, 'status' => 1, 'balance' => $money, 'source' => '1', 'explain' => $message, 'admin_id' => session('userid'), 'create_date' => time()));
            $rel3 = $recharge_table->add(array('uid' => $userInfo['id'],'pursetype'=>$type, 'type' => 5, 'number' => $income, 'admin_id' => session('userid'), 'status' => 1, 'create_date' => time()));
            if ($rel1 && $rel2 && $rel3) {
                $member_table->commit();
                showMsg(1, '操作成功');
            } else {
    ...
    public function deduct() {
        ...
        if (IS_AJAX) {
            ...
            $rel2 = $bonus_table->add(array('uid' => $userInfo['id'],'pursetype'=>$type, 'type' => 6, 'export' => $income, 'status' => 2, 'balance' => $money, 'source' => '1', 'explain' => $message, 'admin_id' => session('userid'), 'create_date' => time()));
            $rel3 = $recharge_table->add(array('uid' => $userInfo['id'],'pursetype'=>$type,  'type' => 6, 'number' => $income, 'admin_id' => session('userid'), 'status' =>2, 'create_date' => time()));
            if ($rel1 && $rel2 && $rel3) {
                $member_table->commit();
                showMsg(1, '操作成功');
            } else {
