...
class ReportController extends CommonController {
    ...
    public function ConfirmReceipt() {
        ...
        $rel2 = $bonus_table->add(array('uid' => $row['uid'], 'sum' => $row['money'], 'balance' => $usermoney, 'explain' => '下线充值', 'source' => '1', 'admin_id' => $_SESSION['userid'], 'type' => '5', 'create_date' => time()));
        $rel3 = $chongzhi_table->save(array('id' => $id, 'status' => '2', 'admin_id' => $_SESSION['userid'], 'replace_date' => time()));
        $rel4 = $recharge_table->add(array('uid' => $row['uid'], 'pursetype' => 1, 'type' => 5, 'number' => $row['money'], 'admin_id' => session('userid'), 'status' => 1, 'create_date' => time()));
    ...
    public function RefuseToPlay() {
        ...
        $rel3 = $bonus_table->add(array('uid' => $row['uid'], 'sum' => $row['money'] , 'pursetype' => $row['pursetype'], 'status' => 1, 'balance' => $usermoney, 'explain' => '拒绝提现', 'type' => '8', 'source' => '3', 'admin_id' => $_SESSION['userid'], 'create_date' => time()));
