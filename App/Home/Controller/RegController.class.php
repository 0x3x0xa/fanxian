...
//            }
            ...
            if ($id) {
                $member_table->commit();
                $member_table->save(array('id' => $rid, 'directnum' => $ruserInfo['directnum'] + 1));
                $this->upline($rid);
                unset($_SESSION['code']);
                //$this->success('恭喜你注册成功',U('Home/Login/index'));
                //showMsg(1, '恭喜你注册成功');

                exit(json_encode(array('status' => '1', 'msg' => '恭喜你注册成功', 'url' => U('Home/Login/index'))));
            } else {
                $pin_table->rollback();
                showMsg(2, '对不起，注册失败!');
            }
