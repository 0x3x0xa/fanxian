...
class ReportController extends CommonController {
    ...
    public function touziguanli() {
        $invest_table = M('invest');
        $uid = session('uid');
        $count = $invest_table->where(array('uid' => $uid))->count();
        $Page = new \Think\Page($count, 16);
        $show = $Page->show();
        $list = $invest_table->where(array('uid' => $uid, 'status' => '1'))->order('create_date desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $count = count($list);
        for ($i = 0; $i < $count; $i++) {
            $row = $this->returnrate($list[$i]['money']);
            $rate = empty($row) ? 0 : $row['rate'];
            $list[$i]['rate'] = $rate;
        }
        $this->assign('count', $count);
        $this->assign('page', $show);
    ...
    public function lishitouzi() {
        $invest_table = M('invest');
        $uid = session('uid');
        $count = $invest_table->where(array('uid' => $uid))->count();
        $Page = new \Think\Page($count, 16);
        $show = $Page->show();
        $list = $invest_table->where(array('uid' => $uid, 'status' => '2'))->order('create_date desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $this->assign('count', $count);
        $this->assign('page', $show);
        $this->assign('list', $list);
        $this->display();
    ...
    public function touzishouyi() {
        ...
        $todaymoney = $shouyi_table->where(array('create_date' => array('egt', $todayTime),'uid'=>$uid))->sum('bonus');
        $todaymoney = empty($todaymoney) ? 0 : $todaymoney;
        $allmoney = $shouyi_table->where(array('uid' => $uid))->sum('bonus');
        $allmoney = empty($allmoney) ? 0 : $allmoney;
        $uid = session('uid');
        $count = $shouyi_table->where(array('uid' => $uid))->count();
        $Page = new \Think\Page($count, 16);
        $show = $Page->show();
        $list = $shouyi_table->where(array('uid' => $uid))->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $this->assign('count', $count);
        $this->assign('page', $show);
        $this->assign('list', $list);
    ...
    public function tuijianbonus() {
        ...
        $todayTime = mktime(0, 0, 0, date("m"), date("d"), date("Y"));
        $todaymoney = $bonusshouyi_table->where(array('create_date' => array('egt', $todayTime), 'type' => 1,'uid'=>$uid))->sum('bonus');
        $todaymoney = empty($todaymoney) ? 0 : $todaymoney;
        $allmoney = $bonusshouyi_table->where(array('uid' => $uid, 'type' => 1))->sum('bonus');
        $allmoney = empty($allmoney) ? 0 : $allmoney;
        $uid = session('uid');
        $count = $bonusshouyi_table->where(array('uid' => $uid, 'type' => 1))->count();
        $Page = new \Think\Page($count, 16);
        $show = $Page->show();
        $list = $bonusshouyi_table->where(array('uid' => $uid, 'type' => 1))->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $this->assign('count', $count);
        $this->assign('page', $show);
        $this->assign('list', $list);
        $this->assign('todaymoney', $todaymoney);
    ...
    public function tuanduibonus() {
        ...
        $bonusshouyi_table = M('bonusshouyi');
        $todayTime = mktime(0, 0, 0, date("m"), date("d"), date("Y"));
        $todaymoney = $bonusshouyi_table->where(array('create_date' => array('egt', $todayTime), 'type' => 2,'uid'=>$uid))->sum('bonus');
        $todaymoney = empty($todaymoney) ? 0 : $todaymoney;
        $allmoney = $bonusshouyi_table->where(array('uid' => $uid, 'type' => 2))->sum('bonus');
        $allmoney = empty($allmoney) ? 0 : $allmoney;
        $uid = session('uid');
        $count = $bonusshouyi_table->where(array('uid' => $uid, 'type' => 2))->count();
        $Page = new \Think\Page($count, 16);
        $show = $Page->show();
        $list = $bonusshouyi_table->where(array('uid' => $uid, 'type' => 2))->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $this->assign('count', $count);
        $this->assign('page', $show);
        $this->assign('list', $list);
        $this->assign('todaymoney', $todaymoney);
    ...
    public function touzi() {
        ...
        if (IS_AJAX) {
            ...
            switch ($types){
                case 1:
                     $usermoney = $userInfo['principal'] - $money;
                    $rel1 = $member_table->save(array('id' => $uid, 'principal' => $usermoney));
                    break;
                case 2:
                     $usermoney = $userInfo['profit'] - $money;
                    $rel1 = $member_table->save(array('id' => $uid, 'profit' => $usermoney));
                    break;
                default:
                    showMsg(2, '类型不存在');
            ...
            if ($minmoney > $money) {
                showMsg(2, '最低投资额不能小于' . $minmoney);
            }
            if ($money % $beishu != 0) {
                showMsg(2, '需要' . $data['benjinbeishu'] . '倍数');
            }
            if (!$member_table->autoCheckToken($_POST)) {
                showMsg(2, '不要重复提交');
            }
            $towpassword = md5pwd(1, $towpassword);
            if ($towpassword != $userInfo['towlevelpassword']) {
                showMsg(2, '二级密码不正确');
            }
            if ($money < 0 || floor($money) != $money || !is_numeric($money)) {
                showMsg(2, '请输入有效的数字');
            ...
            $rel2 = $invest_table->add(array('uid' => $uid, 'no' => build_order_no(),'pursetype'=>$types, 'create_date' => time(), 'money' => $money, 'sum' => $money));
            $rel3 = $bonus_table->add(array('uid' => $uid, 'type' =>$types,'pursetype'=>$types, 'create_date' => time(), 'export' => $money, 'balance' => $usermoney, 'status' => '2', 'explain' => '转入投资'));
            if ($rel1 && $rel2 && $rel3) {
                $this->selftotal($uid, $money);
                if ($userInfo['recommend'] > 0) {
                    $this->grouptotal($userInfo['recommend'], $money);
                }
                $member_table->commit();
                showMsg(1, '操作成功');
            } else {
    ...
    public function tixian() {
        ...
        if (IS_AJAX) {
            ...
            $member_table->startTrans();
            $money = I('post.money', '', 'htmlspecialchars');
            $ids = I('post.id', '', 'htmlspecialchars');
            $towpassword = I('post.towpassword', '', 'htmlspecialchars');
            $userInfo = $member_table->find($uid);
            if (!$member_table->autoCheckToken($_POST)) {
                showMsg(2, '不要重复提交');
            ...
            $usermoney = $userInfo['principal'] + $money;
            $row = $invest_table->find($ids);
            $rel1 = $member_table->save(array('id' => $uid, 'principal' => $usermoney));
            $endmoney = $row['money'] - $money;
            if ($endmoney == 0) {
                $rel2 = $invest_table->save(array('id' => $ids, 'money' => $endmoney, 'replace_date' => time(), 'status' => '2'));
            } else {
                $rel2 = $invest_table->save(array('id' => $ids, 'money' => $endmoney, 'replace_date' => time()));
            }
            $rel3 = $bonus_table->add(array('uid' => $uid, 'pursetype' => 1, 'type' => 7, 'create_date' => time(), 'sum' => $money, 'balance' => $usermoney, 'status' => '2', 'explain' => '投资转出'));
            $totalInfo=$total_table->where(array('uid'=>$uid))->find();
            $rel4=$total_table->save(array('id'=>$totalInfo['id'],'selftotalmoney'=>$totalInfo['selftotalmoney']-$money));
            if ($rel1 && $rel2 && $rel3&&$rel4) {
                if($userInfo['recommend']>0){
                $this->reducegrouptotal($userInfo['recommend'], $money);
                }
                $member_table->commit();
                showMsg(1, '操作成功');
            } else {
