...
<body>
    ...
    <form method="get" action="/index.php/Admin/Report/tixianlist.html">
        <div class="pd-20">
            <div class="text-c"> 
 ...
 &nbsp;&nbsp;&nbsp;&nbsp;
                <span class="select-box" style='width:150px;'>
                    <select name="search_type" class='select' value="">
                        ...
                        <?php if(is_array($types)): foreach($types as $key=>$vo1): ?><option value="<?php echo ($key); ?>" <?php if($arr["search_type"] == $key): ?>selected<?php elseif($_GET['search_type']== $key): ?>selected<?php endif; ?>><?php echo ($vo1); ?></option><?php endforeach; endif; ?>
                    </select></span>
                   <label>
                 ...
                 <th width="100">钱袋</th>
                  <th width="100">金额</th>
                   ...
                   <th width="100">支付金额</th>
                    <th width="100">收款方账号</th>
                  <th width="100">收款方银行</th>
                   <th width="100">收款方账户姓名</th>
                    <th width="100">银行开户网点</th>
                <th width="100">状态</th>
                <th width="180">日期</th>
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
            ...
            <td><?php echo ($vo["bank"]); ?></td>
             <td><?php echo ($vo["username"]); ?></td>
              <td><?php echo ($vo["kaihubank"]); ?></td>
            <td><?php echo ($status[$vo['status']]); ?></td>
            <td><?php echo (date('Y-m-d H:i:s',$vo["create_date"])); ?></td>
           <?php if($vo["status"] == 1): ?><td> 
                    <a href="javascript:;" onclick="FinishedPlaying('<?php echo ($vo["id"]); ?>')" >确认打款</a>  
                    <a href="javascript:;" onclick="RefuseToPlay('<?php echo ($vo["id"]); ?>')" >拒绝</a>
                </td>
                <?php else: ?>
                 <td> 
