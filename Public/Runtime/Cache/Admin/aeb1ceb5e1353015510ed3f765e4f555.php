...
<div class="pd-20">
  <div class="Huiform">
 <form action="/index.php/Admin/Article/articleedit/id/6.html" >
      <table class="table table-bg">
        <tbody>
          <tr>
            <th class="text-r"><span class="c-red">*</span> 标题： </th>
            <td>
                <input type="text" style="width:300px" class="input-text art_title"  value="<?php echo ($article_row["art_title"]); ?>"  placeholder="" id="user-name"
                       name="art_title" datatype="*2-16" nullmsg="标题不能为空">
<!--作者：<input type="text" style="width:118px" class="input-text art_author" 
                       value="<?php echo ($article_row["art_author"]); ?>" placeholder="" id="user-tel" name="art_author"> 
            ...
            <input type="text" style="width:118px" class="input-text art_source"  value="<?php echo ($article_row["art_source"]); ?>"  placeholder="" id="user-tel" name="">-->
			 ...
			 <select class="art_type" id="sel_Sub" name="art_type" onChange="SetSubID(this);" style='width:150px;height:31px;'>
			    <?php if(is_array($list1)): $i = 0; $__LIST__ = $list1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo1["id"]); ?>"><?php echo (htmlspecialchars_decode($vo1["art_class_name"])); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                 ...
                                 发布日期： 
                                 <input id="d11" type="text" onClick="WdatePicker()" class="input-text Wdate"  style="width:120px;" value="<?php echo ($article_row["art_time"]); ?>"/>
          </tr>
