...
function reg() {
    ...
    $.ajax({
        ...
        data: {
            token: token, username: username, password: password,
            name: name,mobile: mobile,code: code,m:m},
        dataType: 'json',
        async: false, //设置为同步操作就可以给全局变量赋值成功 
        success: function (data) {
            if (data.status == 1)
...
function tixianbenjin(id) {
    ...
    $.ajax({
        type: "post",
        url: "/Home/Report/tixian",
        data: {money: money, towpassword: towpassword, token: token, id: id},
        dataType: 'json',
        async: false, //设置为同步操作就可以给全局变量赋值成功 
        success: function (data) {
            if (data.status == 1)
